<div class=" text-white">
    {{-- <h3 class="text-3xl font-bold">Menu Data:</h3>
    <ul class=" ml-5">
        <li>Name: {{ $name }}</li>
        <li>Description: {{ $description }}</li>
        <li>Image: {{ $image }}</li>
        <li>Price: {{ $price }}</li>
        <li>Category ID:{{ $menu_item_category_id }}</li>
        <li class=" mt-3">
            <h3 class=" text-2xl font-bold">Ingredients:</h3>
            @foreach ($ingredients as $ingredient)
                <ul class=" ml-5">
                    @foreach ($ingredient as $column => $data)
                        <li>{{ $column }}: {{ $data }}</li>
                    @endforeach
                </ul>

                @foreach ($alternativeIngredients as $alternative)
                    @if ($ingredient['uid'] == $alternative['ingredient_uid'])
                        <h3 class=" ml-10 text-xl font-semibold">Alternative Ingredients:</h3>
                        <ul class=" ml-10">
                            @foreach ($alternative as $alternativeColumn => $alternativeData)
                                <li>{{ $alternativeColumn }}: {{ $alternativeData }}</li>
                            @endforeach
                        </ul>
                    @endif
                @endforeach
                @if (in_array($ingredient['uid'], $removableIngredients))
                    <span class=" text-lg font-semibold text-red-600">Removable</span>
                @endif
            @endforeach
        </li>
        <li class=" mt-3">
            <h3 class="text-2xl font-bold">Additional Ingredients:</h3>
            @foreach ($additionalIngredients as $additional)
                <ul class=" ml-5">
                    @foreach ($additional as $additionalColumn => $additionalData)
                        <li>{{ $additionalColumn }}: {{ $additionalData }}</li>
                    @endforeach
                </ul>
            @endforeach
        </li>
    ...
    <form wire:submit.prevent="update" class="p-8">
        {{-- MENU INFO --}}
        <div>
                ...
                <div class="flex items-center">
                    <div class="flex items-center justify-center w-96">
                        <label for="image" class="flex flex-col items-center justify-center w-full h-72 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 p-3">
                            ...
                            @else
                                <img src="{{ asset('storage/'. $oldImage) }}" alt="">
                            @endif
